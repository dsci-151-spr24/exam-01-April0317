...
*I, Yihan (April) Wang, hereby state that I have not communicated with or gained information in any way from my classmates or anyone other than the Professor or TA during this exam, and that all work is my own.*
...
flight_counts_with_names <- flight_counts %>%
   left_join(airlines, by = "carrier") %>%
   select(name, number_of_flights)
flight_counts_with_names
...
oringin_airport_name <- airports %>%
  filter(faa == max_delay_flight$origin) %>%
  select(name)
```   
...
Early Morning (12:01am-6am): This time interval might have a lower proportion of delays due to fewer flights at night and lower air and airport traffic volumes.
Morning Rush (6:01am-12pm): As daytime flights begin, air traffic increases, which could lead to a higher proportion of delays.
Afternoon and Evening (12:01pm-6pm and 6:01pm-12am): These intervals are typically when flight volume is high, especially in the evening, which could result in a higher proportion of delays due to accumulated delays, busier airport operations, and adverse weather conditions.
...
flights %>% 
  ...
  ggplot(aes(x = arrival, y = dep_delay, color = dest)) + 
    ...
    labs(title = "On time performance of NYC flights",
         ...
         color = "Destination")
``` 
 The plot shows the departure delay at December 13, comparing the flights to the destination of "PHL", "RDU".
